...
def extract_query_title_from_aksis_data(sentence):
    """extract the query and title from aksis raw data, this function is for building up w2v_data
...
def load_vocabulary_from_pickle(path, top_words=40000, special_words=dict()):
    """load w2v_data from pickle
    Parameters
    ----------
        path: str
            corpus path
        top_words: int
            the max words num in the w2v_data
        special_words: dict
...
def sentence2id(sentence, the_vocab):
    """convert the sentence to the index in w2v_data"""
...
def word2id(word, the_vocab):
    """convert the word to the index in w2v_data"""
...
def convert_data_to_id(encoder_words, decoder_words, encoder_vocab, decoder_vocab):
    """convert the seq2seq data into id which represent the index for word in w2v_data"""
