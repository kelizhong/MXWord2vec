...
def retry(tries, delay=1, backoff=1, exception=Exception, name=None, report=lambda *args: None):
    ...
    if backoff < 1:
        raise ValueError("backoff value must be greater than 1 or equal")
    ...
    def retrying(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if _tries < 0:
                raise ValueError("retry times must be greater than zero")
            if _delay < 0:
                raise ValueError("delay value must be greater than zero")
